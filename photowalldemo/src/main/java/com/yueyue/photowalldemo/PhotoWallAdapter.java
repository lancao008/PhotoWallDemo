...
/**
 * author : yueyue on 2018/3/28 21:01
 * desc   : GridView的适配器，负责异步从网络上下载图片展示在照片墙上。
 */

public class PhotoWallAdapter
        extends ArrayAdapter<Images.ImageInfo>
        implements AbsListView.OnScrollListener {

    private final List<Images.ImageInfo> mList;
    /**
     * 记录所有正在下载或等待下载的任务。
     */
    ...
    public PhotoWallAdapter(Context context, int textViewResourceId, List<Images.ImageInfo> list,
                            GridView photoWall) {
    ...
    private void loadBitmaps(int firstVisibleItem, int visibleItemCount) {
        try {
            for (int i = firstVisibleItem; i < firstVisibleItem + visibleItemCount; i++) {
                String imageUrl = mList.get(i).url;
                Bitmap bitmap = getBitmapFromMemoryCache(imageUrl);
                if (bitmap == null) {
